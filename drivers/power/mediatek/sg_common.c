...
long SG_SocMeasure(void)
{
    ...
    while(g_stSGInfo.nAdcValue[i] != END_OF_ADC_LIST)
    {
		...
		if((++lT3 % ADC_AVG_QUEUE_LENGTH) == 0) //1.5초
		{
			...
			lA4 = lA3 - lA2;
			if(g_bLongTermFlag == TRUE)
			{
				lC1 = SLEEP_CURRENT;
				lT4 = 0;
			}
			else if( g_lgbmChargerType != CHARGER_UNKNOWN && lA4 < 0 )		
			{
...
void SG_AdcLog(void)
{
    ...
    { 
		...
		if((g_lgbmChargerType == CHARGER_UNKNOWN) && (g_lInterval_checker[nLogCount] > 100 * PARAM_SCALE))
		{
			g_bLongTermFlag = TRUE;
		}
...
{	
	...
	get_xtime_and_monotonic_and_sleep_offset(&xts, &tom, &spg_time_before_sleep);
	DEBUG_MSG("[sg_spg_suspend]\n");
	g_is_call_state = LGBM_is_call_state();
	g_bLongTermFlag = TRUE;
    return 0;
}
...
static int sg_spg_resume(struct platform_device *dev)
{
	...
	long lC1 = 0;
	unsigned long lT4 = 0;
	struct timespec xts, tom, spg_time_after_sleep;
#ifdef FEATURE_POWERAPI_SG_ANDROID_LOG
    char sg_msg[200];
#endif

	get_xtime_and_monotonic_and_sleep_offset(&xts, &tom, &spg_time_after_sleep);

	lT4 = (spg_time_after_sleep.tv_sec - spg_time_before_sleep.tv_sec) * PARAM_SCALE;

	if( g_is_call_state == CALL_ACTIVE )
	{
		lC1 = CALL_SLEEP_CURRENT;
	}
	else
	{
		lC1 = SLEEP_CURRENT;
	...
	g_lSocMeas -= lP1;
#ifdef FEATURE_POWERAPI_SG_ANDROID_LOG
    sprintf(sg_msg, "SM: %ld, T: %ld, V: %d, Tmp2: %d Fuel_SOC: %d Current: %d\n",g_lSocMeas, (long)lT4, LGBM_ReadBatVoltAdc(), 77,g_lSocView, lC1);
    SPG_FileLog(fTime, sg_msg);
    fTime += lT4;
#endif
	g_bLongTermFlag = TRUE;
	DEBUG_MSG("[sg_spg_resume]\n");
    return 0;
}
