...
#else
#define LGBM_SW_EOC_VOLTAGE_TH (4200)
...
void LGBM_StateNoCharger( LGBmData *pBmData, LGBmVital *pVital )
{
	...
	g_call_state = LGBM_is_call_state();
	
	if(g_call_state == CALL_ACTIVE && batVolt < LGBM_CUT_OFF_VOLTAGE && g_call_wakelock_enable == KAL_FALSE)
	{
		wake_lock(&battery_suspend_lock);
		g_call_wakelock_enable=KAL_TRUE;
	}
	else if(g_call_wakelock_enable == KAL_TRUE && g_call_state == CALL_IDLE)
	{
		wake_unlock(&battery_suspend_lock);
		g_call_wakelock_enable=KAL_FALSE;
	}
	LGBM_LOG("g_call_state = %d, batVolt = %d  g_call_wakelock_enable= %d \n", g_call_state, batVolt , g_call_wakelock_enable);
			...
			else
			{
				...
				switch( bmData.bmState )
				{
					...
					case LGBM_TS_NO_CHARGER:
						...
						{
							if(g_call_wakelock_enable!=KAL_TRUE)
								wake_unlock(&battery_suspend_lock);
						}
						mt_usb_disconnect();
