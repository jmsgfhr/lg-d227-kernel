...
static void HI707_Initial_Setting(void)
{
	...
	len = sizeof(HI707_Init_Reg) / sizeof(HI707_Init_Reg[0]);
	HI707_table_write_cmos_sensor(HI707_Init_Reg,len);
...
static kal_uint8 HI707_power_on(void)
{
    ...
    do{
        ...
        if(HI707_sensor_id == HI707_SENSOR_ID)
        {
            break;
        }
        retry--;
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    if(HI707_sensor.MPEG4_Video_mode == KAL_TRUE)
    { //15-30fps
        //TODO: Need to "MMS,VGA Check"
        len = sizeof(HI707_Video_Reg[HI707_sensor.banding])/sizeof(HI707_Video_Reg[HI707_sensor.banding][0]);
        HI707_table_write_cmos_sensor(HI707_Video_Reg[HI707_sensor.banding],len);
    }
    else
    { //10-30fps
	...
	case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
		{
			...
			*pHorFOV = 56;  // HorFOV = 58.4
			*pVerFOV = 45;  // VerFOV = 45.18
			*pFeatureParaLen = 8;
		}
		break;
	//                                                                 
